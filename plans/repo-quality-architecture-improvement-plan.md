...
3. Make stub generation explicit.
   ...
   - Align `.gitignore`, `pyproject.toml` `sdist.include`, and wheel workflows
     with that decision.
   - Status: implemented. `_core.pyi` remains ignored and generated by
     nanobind; CI generates it once before sdist/wheel packaging, release
     artifacts include it, and local docs use the same validation script as CI.
...
- Status: partially implemented. Repr/string formatting for `BatchEncoding` now
  ...
  and device migration now live in a dedicated attribute helper module.
