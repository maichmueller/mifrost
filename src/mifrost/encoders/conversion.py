...
from .types import BatchEncodingInput, EncodingDict, PygDataLike
...
def to_pyg(
    ...
    *,
    ...
    """
    Convert native or dictionary encodings into their PyG representation.

    Native encodings use their C++ converter when metadata is requested, while
    dictionary conversions and metadata-suppressed conversions use the Python
    schema converter.
    """
    if (
        include_metadata
        and not isinstance(encoding, Mapping)
        and hasattr(encoding, "as_pyg")
    ):
...
def encoding_to_tensors(
    ...
    """Compatibility alias for ``to_tensor_payload``."""
    return to_tensor_payload(encoding)
