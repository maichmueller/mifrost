...
void BatchBuilder::add_node_features(const std::string& node_type, const std::string& attr_name, std::span<const float> data, int feature_dim)
{
...
void BatchBuilder::ensure_edge_type(const std::string& src_type, const std::string& rel_type, const std::string& dst_type)
{
...
void BatchBuilder::set_node_names(const std::string& node_type, std::vector<std::string> names) { node_names[node_type] = std::move(names); }

void BatchBuilder::set_object_names(std::vector<std::string> names) { object_names = std::move(names); }
...
nb::dict BatchBuilder::build_dict()
{
    ...
    for (auto& [key, col] : columns)
    {
        ...
        std::visit(
            [&](auto&& items)
            {
                ...
                nb::capsule owner(heap_vec, [](void* p) noexcept { delete static_cast<VectorType*>(p); });

                if (is_edge_index)
                {
                    size_t shape[1] = { size };
                    auto tensor = nb::ndarray<nb::numpy, ScalarType, nb::shape<-1>>(heap_vec->data(), 1, shape, owner);
                ...
                auto tensor = nb::ndarray<nb::numpy, ScalarType, nb::shape<-1, -1>>(heap_vec->data(), 2, shape, owner);
    ...
    for (auto& [ntype, p_vec] : ptrs)
    {
        ...
        nb::capsule owner(heap_vec, [](void* p) noexcept { delete static_cast<std::vector<int64_t>*>(p); });

        auto tensor = nb::ndarray<nb::numpy, int64_t, nb::shape<-1>>(heap_vec->data(), 1, shape, owner);
...
nb::object BatchBuilder::build()
{
    ...
    for (const auto& [edge_key, parts] : edge_parts)
    {
        ...
        nb::object edge_index = torch.attr("stack")(nb::make_tuple(parts.src, parts.dst), nb::arg("dim") = 0);
        nb::object store = batch.attr("__getitem__")(nb::make_tuple(std::get<0>(edge_key), std::get<1>(edge_key), std::get<2>(edge_key)));
    ...
    for (const auto& [node_type, ptr] : ptr_vectors)
    {
        ...
        nb::capsule owner(heap_ptr, [](void* p) noexcept { delete static_cast<std::vector<int64_t>*>(p); });
        auto ptr_array = nb::ndarray<nb::numpy, int64_t, nb::shape<-1>>(heap_ptr->data(), 1, shape, owner);
        nb::object ptr_tensor = to_tensor(ptr_array.cast());

        auto batch_it = batch_vectors.find(node_type);
        auto* heap_batch = new std::vector<int64_t>(batch_it != batch_vectors.end() ? batch_it->second : std::vector<int64_t> {});
        size_t batch_shape[1] = { heap_batch->size() };
        nb::capsule batch_owner(heap_batch, [](void* p) noexcept { delete static_cast<std::vector<int64_t>*>(p); });
        auto batch_array = nb::ndarray<nb::numpy, int64_t, nb::shape<-1>>(heap_batch->data(), 1, batch_shape, batch_owner);
    ...
    for (const auto& [node_type, count] : node_counts)
    {
        ...
        if (!has_x)
        {
            ...
            nb::object zeros = torch.attr("zeros")(nb::make_tuple(count, dim), nb::arg("dtype") = torch.attr("float32"));
