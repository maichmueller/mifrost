...
struct RelationDict
{
    ...
    static RelationDict
    build(const mimir::formalism::DomainImpl& domain, const std::vector<mimir::formalism::Action>& actions, const RelationDictConfig& config)
    {
        RelationDict out;
        ...
        for (const auto& [name, arity] : regular_predicates)
        {
            for (const auto& level : levels)
            {
                ...
                {
                    const std::string rel = RelationFormatter::format_predicate(name, level, std::nullopt, polarity);
                    out.arity[rel] = arity;
                }
            }
        }
        ...
        for (const auto& goal_sat : out.goal_satisfaction_derivations)
        {
            for (const auto& [name, arity] : regular_predicates)
            {
                for (const auto& level : levels)
                {
                    for (bool polarity : { true, false })
                    {
                        const std::string rel = RelationFormatter::format_predicate(name, level, goal_sat, polarity);
                        out.arity[rel] = arity;
                    }
                }
            }
