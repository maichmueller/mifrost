...
public:
     ...
     */
    void add_node_features(const std::string& node_type, const std::string& attr_name, std::span<const float> data, int feature_dim);
     ...
     */
    void add_edges(const std::string& src_type,
    ...
    void ensure_edge_type(const std::string& src_type, const std::string& rel_type, const std::string& dst_type);
...
std::vector<T>& BatchBuilder::get_column(const std::string& key, int dim)
{
    ...
    if (it == columns.end())
    {
        ...
        {
            static_assert(std::is_same_v<T, float> || std::is_same_v<T, int64_t>, "Unsupported column type");
        }
    }
