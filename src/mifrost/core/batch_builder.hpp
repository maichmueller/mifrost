...
public:
     ...
     * @param feature_dim Dimension of feature vector (e.g. 1 for simple attributes, N for embeddings)
     */
    void add_node_features(const std::string& node_type, const std::string& attr_name, 
                           std::span<const float> data, int feature_dim);
     ...
     */
    void add_edges(const std::string& src_type, const std::string& rel_type, const std::string& dst_type,
                   std::span<const int64_t> src_indices, std::span<const int64_t> dst_indices);
    ...
    /**
     * @brief Finalize and return a Python dictionary of Tensors (PyG Batch format).
     * 
     * Transfers ownership of vectors to Python (DLPack deleter).
     */
    nb::dict build();
