...
std::pair<int, int>
TransitionDAG::register_transition(mimir::search::State parent, mimir::search::State child, std::optional<mimir::formalism::GroundAction> action)
{
...
bool TransitionDAG::contains(mimir::search::State state) const { return state_to_index_.contains(state); }
        ...
        if (it != adjacency_.end())
        {
            for (int child_idx : it->second)
            {
                if (visited.contains(child_idx))
                {
                    ...
                    nodes_ordered_[child_idx].depth = std::min(nodes_ordered_[child_idx].depth, current_depth + 1);
                }
                else
                {
