...
namespace
{
void append_edges(BatchBuilder& builder, const std::string& src_type, const std::string& rel_type, const std::string& dst_type, int64_t src, int64_t dst)
{
...
HGraphEncoderEngine::HGraphEncoderEngine(const mimir::formalism::DomainImpl& domain) : HGraphEncoderEngine(domain, Config {}) {}
...
void HGraphEncoderEngine::encode_step_impl(const mimir::search::State& state,
...
void HGraphEncoderEngine::encode_state_impl(const mimir::search::State& state, BatchBuilder& builder)
{
    GoalInputs inputs;
...
void HGraphEncoderEngine::encode_impl(const mimir::search::State& state,
                                      ...
                                      BatchBuilder& builder)
{
    ...
    encode_literals(std::span { goals.static_goals }, goals.static_goal_levels, builder, node_indices, node_names, relation_to_symbols, symbol_to_relations);
    encode_literals(std::span { goals.fluent_goals }, goals.fluent_goal_levels, builder, node_indices, node_names, relation_to_symbols, symbol_to_relations);
    encode_literals(std::span { goals.derived_goals }, goals.derived_goal_levels, builder, node_indices, node_names, relation_to_symbols, symbol_to_relations);
    if (!config_.ignore_actions)
    {
        encode_actions(actions, builder, node_indices, node_names, relation_to_symbols, symbol_to_relations);
    }
    if (!goals.static_goals.empty())
    {
        encode_goal_satisfaction(std::span { goals.static_goals },
    ...
    {
        encode_goal_satisfaction(std::span { goals.fluent_goals },
    ...
    {
        encode_goal_satisfaction(std::span { goals.derived_goals },
                                         ...
                                         std::unordered_map<std::string, std::vector<std::string>>& node_names)
{
    ...
    std::sort(ordered.begin(), ordered.end(), [](auto lhs, auto rhs) { return lhs->get_index() < rhs->get_index(); });
    ...
    {
        get_or_add_node(config_.symbol_type_id, config_.nullary_object_name, builder, node_indices, node_names);
    }
}
                                                                  ...
                                                                  std::unordered_map<std::string, std::unordered_set<std::string>>& symbol_to_relations)
{
    ...
    auto handle_atom = [&](auto atom)
    {
        ...
        const auto relation_idx = get_or_add_node(node_type, node_key, builder, node_indices, node_names);
        ...
        for (size_t pos = 0; pos < object_keys.size(); ++pos)
        {
            ...
            const auto obj_idx = get_or_add_node(config_.symbol_type_id, obj_key, builder, node_indices, node_names);
            const std::string pos_str = std::to_string(pos);
    ...
    const auto fluent_atoms = repos.get_ground_atoms_from_indices<mimir::formalism::FluentTag>(state.get_atoms<mimir::formalism::FluentTag>());
    for (const auto& atom : fluent_atoms)
    {
        handle_atom(atom);
    }

    const auto derived_atoms = repos.get_ground_atoms_from_indices<mimir::formalism::DerivedTag>(state.get_atoms<mimir::formalism::DerivedTag>());
                                          ...
                                          std::unordered_map<std::string, std::unordered_set<std::string>>& symbol_to_relations)
{
    for (const auto& literal : goals)
    {
        ...
        const auto predicate = atom->get_predicate();
        const std::optional<int> goal_level = goal_levels.contains(literal) ? std::optional<int>(goal_levels.at(literal)) : std::nullopt;

        const std::string node_type = RelationFormatter::format_predicate(predicate->get_name(), goal_level, std::nullopt, literal->get_polarity());
        const std::string node_key = RelationFormatter::format_literal<GoalTag>(literal, goal_level);
                                                   ...
                                                   std::unordered_map<std::string, std::unordered_set<std::string>>& symbol_to_relations)
{
    for (const auto& goal : goals)
    {
        ...
        std::optional<int> goal_level = goal_levels.contains(goal) ? std::optional<int>(goal_levels.at(goal)) : std::nullopt;

        const std::string node_type = RelationFormatter::format_predicate(predicate->get_name(), goal_level, sat, goal->get_polarity());
        const std::string node_key = RelationFormatter::format_literal<GoalTag>(goal, goal_level, sat);
                                            ...
                                            const std::unordered_map<std::string, std::unordered_set<std::string>>& symbol_to_relations)
{
    ...
    for (const auto& [rel_key, arg_set] : relation_to_symbols)
    {
        ...
        for (const auto& [target_key, tn] : target_to_tn)
        {
            ...
            append_edges(builder, rel_type, config_.lgan_nn_edge_pos, config_.symbol_type_id, rel_idx, sym_idx);
            append_edges(builder, config_.symbol_type_id, config_.lgan_nn_edge_pos, rel_type, sym_idx, rel_idx);
        }
    }
}
...
std::string HGraphEncoderEngine::relation_key(const std::string& node_type, const std::string& node_key) { return node_type + "\n" + node_key; }
...
template void
HGraphEncoderEngine::encode_step_impl<mimir::formalism::StaticTag>(const mimir::search::State& state,
                                                                   std::span<const mimir::formalism::GroundLiteral<mimir::formalism::StaticTag>> goals,
                                                                   std::span<const mimir::formalism::GroundAction> actions,
                                                                   BatchBuilder& builder);
template void
HGraphEncoderEngine::encode_step_impl<mimir::formalism::FluentTag>(const mimir::search::State& state,
                                                                   std::span<const mimir::formalism::GroundLiteral<mimir::formalism::FluentTag>> goals,
                                                                   std::span<const mimir::formalism::GroundAction> actions,
                                                                   BatchBuilder& builder);
template void
HGraphEncoderEngine::encode_step_impl<mimir::formalism::DerivedTag>(const mimir::search::State& state,
