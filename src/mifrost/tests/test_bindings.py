...
def test_batch_builder_basics():
    ...
    assert "atom/x" in batch
    assert "atom|rel|atom/edge_index_0" in batch
    ...
    # Verify edge offsets
    out_src = batch["atom|rel|atom/edge_index_0"]
    # First graph: 0, 1, 2
    # Second graph: 10, 11, 12
    expected_src = torch.cat([src, src + 10])
    assert torch.equal(out_src.flatten(), expected_src)
