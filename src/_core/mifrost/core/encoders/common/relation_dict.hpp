...
/**
 * @brief Relation arity table used by encoders.
 *
 ...
 *
 * Invariant:
 *  - `arity` stores logical relation arities plus any encoder-specific offsets
...
struct RelationDict {
   ...
   bool support_literals = false;
   /// Goal-derived relation variants that are added.
   std::set< GoalDerivation > goal_derivations;
   ...
   )
   {
      ...
      goal_derivations = config.goal_derivations;
