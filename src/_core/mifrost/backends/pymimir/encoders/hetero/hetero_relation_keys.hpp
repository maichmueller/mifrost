/**
 * @file hetero_relation_keys.hpp
 * @brief Mimir-typed structured relation-key builders for the heterogeneous-graph backend.
 *
 * Thin forwarders onto the backend-neutral `RelationKey` builders in
 * `mifrost/core/encoders/common/relation_key.hpp`. Schema declaration and relation-type
 * resolution both build keys through these same functions, so the two can never independently
 * drift on the exported node/edge-type name.
 */
#pragma once
