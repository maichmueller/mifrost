...
struct FlatRelationEncoderEngine::SemanticImpl {
   /// The one engine. It is built from the *domain* schema and never binds to
   /// a problem, so it answers the relation-schema queries (names, arities,
   /// sources, slot roles), finalizes batches, and encodes graphs from any
   /// instance of this domain -- including several in one batch.
   ...
   SemanticImpl(const mimir::formalism::DomainImpl& domain, const Config& config)
       : SemanticImpl(domain, pymimir::build_semantic_schema_context(domain), config)
   {
   }
   ...
   {
      auto& problem_adapter = problems.adapter_for(state);
      const auto problem_context = problem_adapter.get_problem_context();
      ...
      if(goals == nullptr and history.empty() and not history_max_steps.has_value()) {
         ...
         encoder->encode(
            problem_context, state_view, goal_views.goals_view(), action_views, builder
         );
         return;
      }
      ...
      const auto goal_views = goals != nullptr ? problem_adapter.make_goal_views(*goals)
                                               : problem_adapter.make_default_goal_views();
      encoder->encode(
         problem_context,
         state_view,
  ...
  private:
   SemanticImpl(
      ...
      const Config& config
       ...
       : encoder(std::make_unique< SemanticFlatRelationEncoderEngine >(schema, config)),
...
{
   semantic_->encode_impl(state, nullptr, actions, history_subgoals, history_max_steps, builder);
}
...
{
   semantic_->encode_impl(state, &goals, actions, history_subgoals, history_max_steps, builder);
}
...
)
{
   ...
   for(size_t idx = 0; idx < state_count; ++idx) {
      ...
      semantic_->encode_impl(
         state_entry.state, &goals, actions_span, history_span, history_max_steps, builder
      );
      builder.next_graph();
   }
