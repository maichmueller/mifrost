...
public:
    ...
    void encode_state(const mimir::search::State& state, BatchBuilder& builder) override { NB_OVERRIDE_PURE(encode_state, state, builder); }
};
...
NB_MODULE(_core, m)
{
    ...
    nb::class_<StreamEncoderInterface, PyStreamEncoder>(m, "StreamEncoder")
        ...
        .def("encode_state",
             [](StreamEncoderInterface& e, const mimir::search::State& state)
             {
    ...
    nb::class_<BatchBuilder>(m, "BatchBuilder")
        ...
        .def(
            "add_node_features",
            [](BatchBuilder& b, const std::string& type, const std::string& attr, nb::ndarray<float, nb::c_contig> data)
            {
    ...
    nb::class_<GoalInputs>(m, "GoalInputs")
        ...
        .def(nb::init<const std::vector<GoalInputs::AnyGoalLiteral>&, int>(), "goals"_a, "level"_a)
    ...
    nb::class_<HGraphEncoderEngine, StreamEncoderInterface>(m, "HGraphEncoderEngine")
        ...
        .def(
            "encode",
            [](HGraphEncoderEngine& encoder, const mimir::search::State& state, BatchBuilder& builder) { encoder.encode(state, builder); },
            "state"_a,
            "builder"_a)
        .def(
            "encode",
